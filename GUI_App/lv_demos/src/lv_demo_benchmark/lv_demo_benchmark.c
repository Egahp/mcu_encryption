...
static scene_dsc_t scenes[] = {
        {.name = "Rect",                    .weight = 30, .create_cb = rectangle_cb},
        {.name = "Rect rnd",            .weight = 20, .create_cb = rectangle_rounded_cb},
...
static void scene_next_task_cb(lv_timer_t * timer)
{
    ...
    if(scenes[scene_act].create_cb) {
        ...
        if(opa_mode) {
            lv_label_set_text_fmt(subtitle, "\"%s\":%dFPS", scenes[scene_act].name, scenes[scene_act].fps_normal);
        } else {
            if(scene_act > 0) {
                lv_label_set_text_fmt(subtitle, "\"%s+\":%dFPS", scenes[scene_act - 1].name, scenes[scene_act - 1].fps_opa);
            } else {
